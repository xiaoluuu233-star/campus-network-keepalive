...
def build_quickauth_params(config: AppConfig, discovered: Mapping[str, str]) -> dict[str, str]:
    ...
    for key in ("wlanuserip", "wlanacname", "wlanacIp", "mac", "vlan", "groupId"):
...
class PortalClient:
    ...
    def login(self) -> bool:
        ...
        LOGGER.info("发起 Portal 认证: %s", urlencode(redact_params(params)))
        try:
            response = self.session.get(f"{self.config.portal_base_url.rstrip('/')}/quickauth.do", params=params, timeout=self.config.timeout)
